...
if ($result->num_rows > 0) {
 ...
 if($amount>$row["Amount"] or $row["Amount"]-$amount<100){
  $location='detail.php?user='.$sender;
  header("Location: $location&message=transactionDenied");
 }
else{
    $sql = "UPDATE `customers` SET Amount=(Amount-$amount) WHERE Name='$sender'";
...
if($flag==true){
$sql = "UPDATE `customers` SET Amount=(Amount+$amount) WHERE name='$receiver'";
